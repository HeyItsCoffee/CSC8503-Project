...
bool CollisionDetection::RayBoxIntersection(const Ray&r, const Vector3& boxPos, const Vector3& boxSize, RayCollision& collision) {
	...
	for(int i = 0; i < 3; i++){ // Get best three intersections
		if (rayDir[i] > 0)
	...
	const float epsilon = 0.0001f; // An amount of leeway in the calculations
	for (int i = 0; i < 3; i++)
...
bool CollisionDetection::RaySphereIntersection(const Ray&r, const Transform& worldTransform, const SphereVolume& volume, RayCollision& collision) {
	...
	Vector3 dir = (spherePos - rayPos);

	// Then project the sphere's origin onto our ray direction vector
	float sphereProj = Vector3::Dot(dir, rayDir);

	if (sphereProj < 0.0f) return false; // Point is behind the ray

	// Get closest point on ray line to sphere
	Vector3 point = rayPos + (rayDir * sphereProj);

	float sphereDist = (point - spherePos).Length();

	if (sphereDist > sphereRadius) return false;

	float offset = sqrt(pow(sphereRadius, 2) - pow(sphereDist, 2));

	collision.rayDistance = sphereProj - offset;
	collision.collidedAt = rayPos + (rayDir * collision.rayDistance);
	return true;
}
...
// Returns true if object B is fully incased within object A, or vice versa
bool CollisionDetection::AABBFullyWithin(const Vector3& posA, const Vector3& posB, const Vector3& halfSizeA, const Vector3& halfSizeB) {
	Vector3 deltaPos = posB - posA;
	Vector3 deltaSize = (halfSizeB - halfSizeA);

	if (abs(deltaPos.x) < abs(deltaSize.x) &&
		abs(deltaPos.y) < abs(deltaSize.y) &&
		abs(deltaPos.z) < abs(deltaSize.z))
		return true;
	return false;
}
...
Ray CollisionDetection::BuildRayFromObjectFacing(GameObject& object) {
	Transform worldTransform = object.GetTransform();
	Quaternion orientation = worldTransform.GetOrientation();
	
	Vector3 facing = orientation * Vector3(0, 0, -1);

	Vector3 position = worldTransform.GetPosition();

	return Ray(position, facing);
}
