...
// Instead of moving camera based on keyboard input, have it follow the goat (position given as extra parameter)
void Camera::UpdateCamera(float dt, Vector3 goatPos, Vector3 goatSize) {
	//Update the mouse by how much
	pitch -= (Window::GetMouse()->GetRelativePosition().y);
	yaw -= (Window::GetMouse()->GetRelativePosition().x);
	...
	// Get rotation around the goat
	Vector3 rotation = Matrix3::FromEuler(Vector3(0, yaw, pitch)) * Vector3(0, 0, goatSize.y * 5);

	// Calculate position for the camera, based off prev position, and new goat position

	Vector3 positionDifference = goatPos - lastTargetPosition;

	positionDifference *= (dt + 0.05);

	position = lastTargetPosition + positionDifference + rotation + Vector3(0, goatSize.y * 0.25, 0);

	// Update the last goat position
